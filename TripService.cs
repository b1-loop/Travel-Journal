...
using Spectre.Console;
    ...
    namespace Travel_Journal
    {
        ...
        // === ADD UPCOMING TRIP MED UNDO/REDO ===
        public void AddUpcomingTrip()
        {
            // Snygg rubrik
            UI.Transition("Add Upcoming Trip ✈️");

            // 1) Start-state för Undo/Redo – tom resa med rimliga standardvärden
            var initialDraft = new TripDraft(
                country: "",
                city: "",
                budget: 0m,
            ...
            // Skapa Undo/Redo-manager
            var history = new UndoRedoManager<TripDraft>(initialDraft);

            bool editing = true; // styr loopen

            while (editing)
            {
                Console.Clear(); // nollställer vyn varje varv

                // 2) Hämta nuvarande state från historiken
                var current = history.CurrentState;

                // Visa nuvarande utkast
                AnsiConsole.MarkupLine("[bold underline]Current upcoming trip[/]");
                AnsiConsole.MarkupLine($"[grey]Country:[/] {current.Country}");
                AnsiConsole.MarkupLine($"[grey]City:[/] {current.City}");
                AnsiConsole.MarkupLine($"[grey]Budget:[/] {current.Budget}");
                AnsiConsole.MarkupLine($"[grey]Start date:[/] {current.StartDate:yyyy-MM-dd}");
                AnsiConsole.MarkupLine($"[grey]End date:[/] {current.EndDate:yyyy-MM-dd}");
                ...
                var choice = AnsiConsole.Prompt(
                    new SelectionPrompt<string>()
                        ...
                        .AddChoices(
                            ...
                            "4) What dates do you want to travel",
                            "5) Undo last change",
                            "6) Regret last change",
                            "7) Save trip",
                            "8) Cancel")
                );
                ...
                switch (choice)
                {
                    case "1) Choose a country":
                        {
                            // Användaren skriver nytt land
                            string newCountry = AnsiConsole.Ask<string>("New [bold]country[/]:");

                            // Skapa ett nytt state där BARA country ändras
                            var newState = new TripDraft(
                                newCountry,
                                current.City,
                                current.Budget,
                                current.StartDate,
                            ...
                            // Registrera ändringen (pushar gamla state till Undo-stack)
                            history.ApplyChange(newState);
                            break;
                        }

                    case "2) Choose a city":
                        {
                            string newCity = AnsiConsole.Ask<string>("New [bold]city[/]:");
                    ...
                    case "3) What is your budget":
                        {
                            // Din egna metod med felhantering för decimal
                            decimal newBudget = AskDecimal("New [bold]budget[/]:");
                    ...
                    case "4) What dates do you want to travel":
                        {
                            var newStart = AnsiConsole.Ask<DateTime>("New [bold]start date[/] (yyyy-MM-dd):");
                            var newEnd = AnsiConsole.Ask<DateTime>("New [bold]end date[/] (yyyy-MM-dd):");
                    ...
                    case "5) Undo last change":
                        {
                            // Försök ångra – om det inte finns något att ångra, visa fel
                            if (!history.Undo())
                                UI.Error("Nothing to undo.");
                            break;
                        }

                    case "6) Regret last change":
                        {
                            // Försök göra om – om inget att göra om, visa fel
                            if (!history.Redo())
                                UI.Error("Nothing to redo.");
                            break;
                        }

                    case "7) Save trip":
                        {
                            // 4) Spara nuvarande state som en riktig Trip
                            var final = history.CurrentState;

                            // Enkel validering
                            if (string.IsNullOrWhiteSpace(final.Country) ||
                                string.IsNullOrWhiteSpace(final.City))
                            {
                                UI.Error("Country and city cannot be empty.");
                                break;
                            }

                            // Skapa Trip-objekt – vi använder object initializer
                            var trip = new Trip
                            {
                                Country = final.Country,
                                City = final.City,
                                PlannedBudget = final.Budget,
                            ...
                            // Lägg till och spara
                            trips.Add(trip);
                            SaveTrips(); // eller store.Save(trips); beroende på din implementation

                            AnsiConsole.MarkupLine("[green]✅ Upcoming trip saved![/]");
                            editing = false; // avsluta loopen
                            break;
                        }
